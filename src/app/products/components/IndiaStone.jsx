...
import { indiaStoneData } from '@/app/data/indiaStoneData';

export default function IndiaStone() {   
    const [selectedProduct, setSelectedProduct] = useState(null);

    const products = indiaStoneData;
    ...
    return (
        <div className="container mx-auto py-10 px-4">
            <h1 className="text-3xl font-bold text-center mb-8 text-black-600">DÒNG ĐÁ NUNG KẾT ẤN ĐỘ</h1>
            ...
            <div className="mt-20">
                <div className="flex items-center justify-center mb-8">
                    ...
                    <h4 className="text-2xl font-medium text-center px-6">Hình Ảnh Sản Phẩm Hoàn Thiện</h4>
                    <div className="h-0.5 bg-gray-200 flex-grow max-w-xs"></div>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-8">
                    {products.map((product) => (
                        product.demoImages && product.demoImages.map((img, index) => (
                            <div key={`${product.id}-demo-${index}`} className="relative h-64 sm:h-80 shadow-lg rounded-lg overflow-hidden group cursor-pointer">
                                <Image 
                                    src={img} 
                                    alt={`${product.name} - Demo ${index + 1}`} 
                                    fill
                                    sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 33vw"
                                    style={{objectFit: 'cover'}}
                                    className="group-hover:scale-105 transition-transform duration-700"
                                />
                                <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-transparent to-transparent opacity-70 sm:opacity-0 sm:group-hover:opacity-70 transition-opacity duration-300 flex items-end">
                                    <div className="p-4 w-full">
                                        <p className="text-white font-medium text-sm sm:text-base">{product.name}</p>
                                        <p className="text-gray-300 text-xs sm:text-sm mt-1">{product.applications[0]}</p>
                                    </div>
                                </div>
                            </div>
                        ))
  ...
  <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 p-2 sm:p-4">
    <div className="bg-white rounded-xl max-w-5xl w-full h-[90vh] md:h-auto p-0 relative overflow-hidden shadow-2xl">
      ...
      <div className="flex flex-col md:flex-row h-full md:h-auto">
        {/* Hình ảnh - sửa tại đây */}
        <div className="w-full md:w-1/2 bg-gray-50 aspect-video md:aspect-[4/5] relative">
